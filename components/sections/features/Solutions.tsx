...
export function Solutions() {
    ...
    return (
        <section
            ref={sectionRef}
            id="solutions"
            className="section-reveal px-6 py-32 relative overflow-hidden bg-white dark:bg-transparent transition-colors duration-500"
        >
            ...
            <div className="max-w-7xl mx-auto relative">
                <motion.h2
                    ...
                    className="text-4xl md:text-5xl font-semibold tracking-tight mb-20 text-center text-gray-900"
                >
                    {sectionTitle}
                </motion.h2>
                ...
                <div className="services-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {services.map((service, i) => (
                        <PremiumCard
                            key={i}
                            className={cn(
                                "service-card group relative p-6 transition-all duration-500 will-change-transform bg-white/5 border border-black/5 shadow-lg hover:shadow-2xl hover:-translate-y-2",
                        ...
                        >
                            ...
                            <div className="h-40 mb-6 rounded-xl bg-gray-50/5 flex items-center justify-center overflow-hidden relative border border-black/5 group-hover:border-transparent transition-colors">
                                ...
                                <div className="flex flex-wrap gap-2 justify-center px-4">
                                    {service.features[locale].slice(0, 3).map((feature, idx) => (
                                        <span key={idx} className="px-2 py-1 rounded-full bg-black/5/10 text-[10px] font-medium text-gray-600">
                                            {feature}
                                        </span>
                                    ))}
                                </div>
                            </div>
                            <h3 className="text-2xl font-semibold mb-3 text-gray-900">{service.title[locale]}</h3>
                            ...
                            <div className="flex items-center justify-between mt-auto">
                                ...
                                <button className="w-10 h-10 rounded-full bg-black text-white dark:text-black flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-[-10px] group-hover:translate-x-0 hover:scale-110">
                                    →
                                </button>
                            </div>
                        </PremiumCard>
