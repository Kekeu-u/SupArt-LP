...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 px-8 py-2 sm:p-4 transition-all duration-300">
      <div className="container mx-auto">
        <div className="flex flex-col items-center md:flex-row md:justify-center">
            ...
            <div 
              className={`flex justify-between items-center bg-black/20 backdrop-blur-lg rounded-full border border-white/10 shadow-lg px-2 sm:px-4 transition-all duration-300 ${isIconOnly ? 'py-2' : 'py-4'}`}
            >
                <a 
                  href="#home" 
                  onClick={(e) => handleLinkClick(e, '#home')} 
                ...
                <nav className={`flex items-center justify-center transition-all duration-300 ${isIconOnly ? 'gap-x-1 sm:gap-x-2' : 'gap-x-2 md:gap-x-4'}`}>
                    {navLinks.map((link) => (
                      <a
                        ...
                        className="flex items-center gap-x-2 text-gray-300 hover:text-white transition-all duration-300 rounded-full px-1 py-1 sm:px-3 sm:py-2 hover:bg-white/10"
                        title={link.label}
                ...
                <a 
                    ...
                    className={`bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-full transition-all duration-300 transform hover:scale-105 whitespace-nowrap ml-2 sm:ml-4 ${isIconOnly ? 'px-3 py-2 text-sm' : 'px-6 py-3 text-base'}`}
                >
                    Orçamento
                </a>
            </div>
