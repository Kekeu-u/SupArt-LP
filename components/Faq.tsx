    ...
    <div className="border-b border-slate-200 dark:border-white/10">
      ...
      >
        ...
        <span className={`transform transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>
          <svg className="w-6 h-6 text-violet-500 dark:text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
        </span>
      </button>
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96' : 'max-h-0'}`}>
        <p className="text-gray-600 dark:text-gray-300 pb-6 pr-6">{answer}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <section id="faq" className="py-20 px-6">
      <div className="container mx-auto max-w-3xl">
        <h2 className="text-4xl font-bold text-center mb-4">Dúvidas Frequentes</h2>
        ...
        <div className="bg-white/30 dark:bg-white/5 backdrop-blur-md p-4 sm:p-8 rounded-2xl border border-slate-200 dark:border-white/10">
